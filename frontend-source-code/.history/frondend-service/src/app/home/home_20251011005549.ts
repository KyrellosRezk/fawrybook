...
export class Home implements OnInit {
  ...
  follow(suggestion: SuggestionFriend) {
    this.http.post(`http://localhost:8080/user-management/api/v1/follow/${suggestion.id}`, {})
      .subscribe({
        next: () => {
          this.suggestionsPage.content = this.suggestionsPage.content.filter(s => s.id !== suggestion.email);
        },
  ...
  cancel(suggestion: SuggestionFriend) {
    this.http.post(`http://localhost:8080/user-management/api/v1/unfollow/${suggestion.id}`, {})
      .subscribe({
        next: () => {
          this.suggestionsPage.content = this.suggestionsPage.content.filter(s => s.id !== suggestion.id);
        },
        error: err => console.error(err)
      });
  }
