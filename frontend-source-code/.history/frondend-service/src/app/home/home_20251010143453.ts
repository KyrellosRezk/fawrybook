...
export class Home implements OnInit {
  ...
  constructor(private auth: Auth, private http: HttpClient, private router: Router) {}
    ...
    this.loadingUser = false; // user is loaded
    ...
    this.http.post<Page<PostResponse>>(
      ...
      next: (data) => {
        if (append) {
          ...
          this.postsPage.content = data.content;
        }
  ...
  follow(suggestion: SuggestionFriend) {
    this.http.post(`http://localhost:8080/user-management/api/v1/follow/${suggestion.email}`, {}).subscribe({
      next: () => {
  ...
  cancel(suggestion: SuggestionFriend) {
    this.http.post(`http://localhost:8080/user-management/api/v1/unfollow/${suggestion.email}`, {}).subscribe({
      next: () => {
