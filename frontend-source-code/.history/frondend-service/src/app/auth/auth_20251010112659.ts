...
export class Auth {
  ...
  signout() {
    ...
    return this.http.post(`${this.baseUrl}/sign-out`, {}, {headers}).subscribe({
      next: () => {
  ...
  const accessToken = localStorage.getItem('accessToken');
  if (!accessToken) return throwError(() => new Error('No access token'));

  const headers = new HttpHeaders().set('Authorization', `Bearer ${accessToken}`);

  return this.http.post<SignInResponse>(`${this.baseUrl}/token-signin`, { headers }).pipe(
    tap((response: SignInResponse) => {
      // Save tokens if new ones are returned
      if (response.accessToken) localStorage.setItem('accessToken', response.accessToken);
      if (response.refreshToken) localStorage.setItem('refreshToken', response.refreshToken);
      if (response.OTPToken) localStorage.setItem('otpToken', response.OTPToken);
    }),
    catchError((err) => {
      // If token is expired, remove and redirect to login
      if (err.status === 401) {
        localStorage.removeItem('accessToken');
        localStorage.removeItem('refreshToken');
        localStorage.removeItem('otpToken');
