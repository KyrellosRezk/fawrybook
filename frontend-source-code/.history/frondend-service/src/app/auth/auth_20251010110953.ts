...
export class Auth {
  ...
  verifyOtp(payload: VerifyOtpRequest): Observable<SignInResponse> {
   ...
   refreshToken(): Observable<SignInResponse> {
    ...
    return this.http.post<SignInResponse>(`${this.baseUrl}/refresh-token`, {}, { headers }).pipe(
      tap((response: SignInResponse) => {
        if (response.accessToken) localStorage.setItem('accessToken', res.accessToken);
        if (reresponses.refreshToken) localStorage.setItem('refreshToken', res.refreshToken);
      })
    );
  }
  ...
  resetOtp(headers: HttpHeaders): Observable<OTPResponse> {
